...
| Report a **security vulnerability** | **Not an issue.** Use GitHub's private vulnerability reporting - see [Reporting a vulnerability](docs/security.md#reporting-a-vulnerability) |
...
rather than toward a coverage percentage - token rotation and reuse detection, permission
expansion, cross-tenant isolation, owner-lockout prevention, ledger balance under
concurrency, secret redaction.
...
backend job was removed when builds moved to Render and Vercel, so `ruff`, `mypy --strict`,
`pytest` and `alembic check` block nothing on their own. A red backend merges unless you
catch it here.
