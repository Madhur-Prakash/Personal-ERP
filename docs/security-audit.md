...
All 19 public routes are pre-auth by necessity: `/health/{live,ready}` and `/health`;
`/auth/{login, login/2fa, register, refresh, verify-email, resend-verification,
forgot-password, reset-password, magic-link, magic-link/verify, magic-link/device,
magic-link/device/poll, otp, otp/verify, password-policy}`; and
